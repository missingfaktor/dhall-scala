...
class DhallParser private[dhall](val input: ParserInput) extends Parser {
  ...
  def Expression: Rule1[Expression[Nothing, Path]] = rule {
    EnvExpression |
    UrlExpression |
    FileExpression |
  ...
  def ListLiteralExpression: Rule1[ListLit[Nothing, Path]] = rule {
    (ws("[") ~ zeroOrMore(Expression).separatedBy(",") ~ ws("]")) ~> ((xs: Seq[Expression[Nothing, Path]]) => ListLit(None, xs))
  }

  def AlternativeType: Rule1[(String, Expression[Nothing, Path])] = rule {
    (capture(oneOrMore(Identifier)) ~ ws(":") ~ Expression) ~> ((k: String, v: Expression[Nothing, Path]) => (k, v))
  }
